...
jQuery(document).ready(function($) {
	...
	// Wait for DOM and sticky plugin to be ready
...
function initNewsSection() {
  // Add smooth scrolling for news links
  $('a[href="#news-section"]').on('click', function(e) {
    e.preventDefault();
    $('html, body').animate({
  ...
  // Add click functionality for news links
  $('.news-link').on('click', function(e) {
    e.preventDefault();
    // Here you can add functionality to show full news article
    // For now, we'll just show an alert
    const newsTitle = $(this).closest('.news-content').find('h4').text();
    alert('Открытие новости: ' + newsTitle + '\n\nВ будущем здесь будет отображаться полный текст новости.');
  });

  // Add animation for news cards on scroll
  $(window).on('scroll', function() {
    $('.news-card').each(function() {
      ...
      if (elementBottom > viewportTop && elementTop < viewportBottom) {
        $(this).addClass('news-card-visible');
      }
    });
  });
}
...
function initAboutSection() {
  // Add smooth scrolling for about links
  $('a[href="#about-section"]').on('click', function(e) {
    e.preventDefault();
    $('html, body').animate({
  ...
  // Add hover effects for about features
  $('.about-feature').hover(
    function() {
  ...
  // Value items are now static (no click functionality)

}

// Initialize about section when document is ready
$(document).ready(function() {
  initNewsSection();
  initAboutSection();
});
